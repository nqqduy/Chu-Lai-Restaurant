...
let voucherUsed = [];
checkVoucher = () => {
    voucherEnter = enternvoucher.value;
    console.log(voucherEnter);
    if (voucherEnter.length > 0) {
        firebase
            .database()
            .ref('voucher/' + voucherEnter)
            .once('value', data => {
                if (data.exists()) {
                    let check = false;

                    for (var i = 0; i < voucherUsed.length; i++) {
                        if (voucherUsed[i] == voucherEnter) {
                            check = true;
                        }
                    }
                    if (!check) {
                        voucherUsed = voucherUsed.concat(voucherEnter);
                        let giaGiam = priceTotal * 1 - data.val().gia * 1;
                        priceTotal = giaGiam;
                        if (giaGiam < 0) priceTotal = 0;

                        totalPrice.innerHTML =
                            'Tổng cộng giá đã đặt: ' + priceTotal + ' VND';
                    }
                } else alert('voucher không hợp lệ, vui lòng nhập lại');
            });
    } else {
...
checkout = () => {
    firebase
        ...
        .set({
            tonggia: String(priceTotal),
            phuongthucthanhtoan: 'Trả sau',
        })
        .then(data => {
            alert('Đến trang thanh toán, điền thông tin!!');
